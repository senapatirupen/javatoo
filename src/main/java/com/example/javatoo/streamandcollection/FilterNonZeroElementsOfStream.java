...
public class FilterNonZeroElementsOfStream {
    public static void main(String[] args) {
        List<Integer> ints = Arrays.asList(1, 2, -4, 0, 2, 0, -1, 14, 0, -1);
        List<Integer> result1 = ints.stream().filter(i -> i != 0).collect(Collectors.toList());
        ...
        List<Integer> result3 = ints.stream()
                .filter(FilterNonZeroElementsOfStream::evenBetween0And10)
                .collect(Collectors.toList());
